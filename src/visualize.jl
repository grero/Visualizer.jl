...
function visualize(X,width::Integer, height::Integer,title::String; overlay::Bool=false)
    ...
    Tk.grid(c,1,1, sticky="nsew")
    fctrls = Frame(fwin)
    Tk.grid(fctrls,2,1,sticky="sw",padx=5,pady=5)
    Tk.grid_columnconfigure(fwin,1,weight=1)
    Tk.grid_rowconfigure(fwin,1,weight=1)
...
function plottest(c,navstate::NavigationState,navctrls::NavigationControls,X,i::Integer,plotfunc::Function)
    ...
    elseif typeof(p) <: Table
        for cc=1:p.cols
            for rr=1:p.rows
				if isdefined(p.content,sub2ind((p.rows,p.cols),rr,cc))
